'use strict';

/**
 * Update heard about
 */

const Joi = require('joi');
const Boom = require('boom');
const MongoDB = require('mongodb');

// Validation schema for params
const schemaParams = Joi.object({
    _id: Joi.string().length(24).hex().required()
});
// Validation schema for payload
const schemaPayload = Joi.object({
    label: Joi.string().trim(),
    label__fr: Joi.string().trim().allow(null),
}).min(1);

/**
 * Export route to update heard about
 */
module.exports = {
    method: 'PATCH',
    path: '/heard-about/{_id}',
    config: {
        validate: {
            params: schemaParams,
            payload: schemaPayload
        },
        description: 'Route to update heard about',
        tags: ['heard-about', 'update']
    },
    handler: async (request, h) => {

        // Get updates from payload
        const updates = { $set: request.payload };

        // Get heard about id
        const _id = new MongoDB.ObjectId(request.params._id);

        // @hook update:before-update:heard-about

        // Insertion options
        const options = { w: 'majority' };

        // Update heard about in database
        const result = await request.server.db.collection('heard_about')
            .findOneAndUpdate({ _id }, updates, options);

        if (result.value === null) {
            return Boom.notFound('Heard About not found');
        }

        return h.response().code(204);
    }
};

