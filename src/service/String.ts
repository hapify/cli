...
export class StringService {
	...
	public removeDiacritics(value: string): string {
		for (let i = 0; i < this.defaultDiacriticsRemovalMap.length; i++) {
			value = value.replace(
				this.defaultDiacriticsRemovalMap[i].letters,
	 ...
	 * @returns {string}
	 */
	public replaceNonAlphaNumericChars(
		value: string,
		separator: string = ' '
	): string {
	 ...
	 * @returns {string}
	 */
	public replaceNonAlphaNumericCharsButUnderscore(
		value: string,
		separator: string = ' '
	): string {
	 ...
	 * @returns {string}
	 */
	public splitCamelCase(value: string): string {
		return value
			.trim()
			.split(/(?=[A-Z])/)
			.join(' ');
	 ...
	 * @returns {string}
	 */
	public upperCaseWords(value: string): string {
		return value
			.split(' ')
			.map((word: string) => {
	 ...
	 * @returns {string}
	 */
	public format(value: string, format: SentenceFormat): string {
		// Trim
		value = value.trim();
		...
		if (
			format === SentenceFormat.SlugUnderscore ||
			format === SentenceFormat.SlugUnderscoreUpperCase
		) {
			// Use only spaces
		...
		if (format === SentenceFormat.WordsLowerCase) {
			return value;
		}
		...
		if (format === SentenceFormat.WordsUpperCase) {
			return value;
		}

		const camel = this.removeSpaces(value);

		if (format === SentenceFormat.UpperCamelCase) {
			return camel;
		}

		if (format === SentenceFormat.LowerCamelCase) {
