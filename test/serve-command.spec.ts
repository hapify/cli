...
describe('serve command', () => {
	...
	it('no token', async () => {
		await expect(SingleUseWebSocketClient(authJson.url.split('token=')[0], { id: 'get:info', data: {} })).to.reject('Unexpected server response: 401');
	});
	...
	// =======================================================================================
	it('get info', async () => {
		const response = await SingleUseWebSocketClient<{}, WebSocketGetInfoHandlerOutput>(authJson.url, { id: 'get:info', data: {} });
	...
	it('get models', async () => {
		const response = await SingleUseWebSocketClient<{}, IModel[]>(authJson.url, { id: 'get:models', data: {} });
	...
	it('get presets', async () => {
		const response = await SingleUseWebSocketClient<{}, IPreset[]>(authJson.url, { id: 'get:presets', data: {} });
		expect(response).to.be.an.array();
	...
	it('get channels', async () => {
		const response = await SingleUseWebSocketClient<{}, IChannel[]>(authJson.url, { id: 'get:channels', data: {} });
		expect(response).to.be.an.array();
		expect(response.length).to.least(1);
		expect(response[0].name).to.be.a.string();
	...
	it('path preview', async () => {
		...
		const response = await SingleUseWebSocketClient<WebSocketPathPreviewHandlerInput, string>(authJson.url, {
			id: 'prv:path',
			data: {
				model: models[0].id,
		...
		const response2 = await SingleUseWebSocketClient<WebSocketPathPreviewHandlerInput, string>(authJson.url, {
			id: 'prv:path',
			data: {
	...
	it('template preview', async () => {
		...
		// With one model
		const response = await SingleUseWebSocketClient<WebSocketTemplatePreviewHandlerInput, IGeneratorResult>(authJson.url, {
			id: 'prv:template',
			data: {
				model: models[0].id,
		...
		// With all models
		const response2 = await SingleUseWebSocketClient<WebSocketTemplatePreviewHandlerInput, IGeneratorResult>(authJson.url, {
			id: 'prv:template',
			data: {
				channel: channels[0].id,
	...
	it('validate model', async () => {
		...
		// Working validation
		const response = await SingleUseWebSocketClient<WebSocketValidateModelHandlerInput, Validator>(authJson.url, {
			id: 'val:model',
			data: {
				model: models[0],
		...
		const response2 = await SingleUseWebSocketClient<WebSocketValidateModelHandlerInput, Validator>(authJson.url, {
			id: 'val:model',
			data: {
				model: models[0],
				content: channels[0].validator,
		...
		expect(response2).to.equal({
			warnings: [],
			errors: ['Primary key is required', 'Model with owner access must have a ownership field'],
		});
	});
	...
	it('apply preset', async () => {
		const presets = await SingleUseWebSocketClient<{}, IPreset[]>(authJson.url, { id: 'get:presets', data: {} });
		const preset = presets.find((p) => p.name === 'Listing');
		const response = await SingleUseWebSocketClient<WebSocketApplyPresetHandlerInput, WebSocketApplyPresetHandlerOutput>(authJson.url, {
			id: 'apply:presets',
			data: {
	...
	it('generate template', async () => {
		const channels = await SingleUseWebSocketClient<{}, IChannel[]>(authJson.url, { id: 'get:channels', data: {} });
		const response = await SingleUseWebSocketClient<WebSocketGenerateTemplateHandlerInput, void>(authJson.url, {
			id: 'gen:template',
			data: {
				channel: channels[0].id,
	...
	it('generate channel', async () => {
		const channels = await SingleUseWebSocketClient<{}, IChannel[]>(authJson.url, { id: 'get:channels', data: {} });
		const response = await SingleUseWebSocketClient<WebSocketGenerateChannelHandlerInput, void>(authJson.url, {
			id: 'gen:channel',
			data: {
				channel: channels[0].id,
			},
	...
	it('new model', async () => {
		const response = await SingleUseWebSocketClient<WebSocketNewModelHandlerInput, IModel>(authJson.url, {
			id: 'new:model',
			data: {
				name: 'Profile',
			},
		...
		expect(response.fields.map((f) => f.name)).to.contains(['_id', 'created at']); // Default fields
	});
	...
	it('set channels', async () => {
		...
		const response = await SingleUseWebSocketClient<IChannel[], void>(authJson.url, {
			id: 'set:channels',
			data: [newChannel],
		});
		expect(typeof response).to.equal('undefined');
	...
	it('set models', async () => {
		...
		const response = await SingleUseWebSocketClient<IModel[], void>(authJson.url, {
			id: 'set:models',
		...
		// Check from endpoint
		const response2 = await SingleUseWebSocketClient<{}, IModel[]>(authJson.url, { id: 'get:models', data: {} });
